...
- **`e2e update`** — Re-copy managed spec files and regenerate `tsconfig.e2e.json` after upgrading wp-env-bin
...
└── e2e/
    ...
    ├── tsconfig.json                     # Base TypeScript config for e2e specs
    ├── tsconfig.e2e.json                 # Generated — path aliases pointing to wp-env-bin lib (regenerated by `e2e update`)
    ...
    └── specs/
        ...
        ├── editor/
        │   └── blocks.spec.ts            # Managed — discovery editor tests; refresh with `e2e update`
        └── frontend/
            └── blocks.spec.ts            # Managed — discovery frontend tests; refresh with `e2e update`
```
