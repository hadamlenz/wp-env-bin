...
function assertMarkupAttribute(content, attrName, attrConfig, exampleValue) {
    ...
    if (hasDefault && JSON.stringify(exampleValue) === JSON.stringify(defaultVal)) {
        expect(content, `"${attrName}" example value matches block.json default — WordPress omits it from markup`).not.toContain(`"${attrName}"`);
        return;
    }
...
function registerEditorTests(test, config) {
    ...
    test.describe(`${title} - Editor`, () => {
        ...
        test.afterEach(async ({ requestUtils }) => {
            if (postId) {
                await (0, helpers_js_1.deletePost)(requestUtils, postId);
                postId = undefined;
            }
        });
        // -----------------------------------------------------------------------
        ...
        test('block can be inserted via the block inserter', async ({ admin, editor, page, requestUtils }) => {
            ...
            postId = await (0, helpers_js_1.createPostAndGetId)(admin, page, requestUtils);
            ...
            await expect(editor.canvas.getByRole('document', { name: blockSelector }).first()).toBeVisible();
        });
        test('block renders visibly in the editor canvas', async ({ admin, editor, page, requestUtils }) => {
            ...
            await expect(editor.canvas.getByText(/This block has encountered an error/i)).not.toBeVisible();
        ...
        test('block serializes with the correct wp comment delimiter', async ({ admin, editor, page, requestUtils }) => {
            ...
            await (0, helpers_js_1.waitForEditorReady)(page);
        ...
        test('block has no validation errors after save and reload', async ({ admin, editor, page, requestUtils }) => {
            ...
            await page
                ...
                .waitFor({ state: 'visible' });
            await expect(page.getByText('This block contains unexpected or invalid content')).not.toBeVisible();
        });
        // -----------------------------------------------------------------------
        // Block operations
        //
        ...
        test('block can be duplicated without errors', async ({ admin, editor, page, requestUtils }) => {
            ...
            await page.waitForFunction(([blockType, expectedCount]) => {
                const flattenBlocks = (blocks) => blocks.flatMap((b) => [b, ...flattenBlocks(b.innerBlocks ?? [])]);
...
function registerEditorTestsFromConfig(test, configPath) {
    ...
    for (const dir of (cfg.editor ?? [])) {
        ...
        registerEditorTests(test, (0, block_loader_js_1.loadEditorConfig)(blockJsonPath));
    }
}
