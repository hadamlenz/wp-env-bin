 ...
 */
export function sanitizeAttributesForApi(exampleAttributes, blockAttributes) {
 ...
 */
export function resolveBlockCss(blockJsonPath, blockJson) {
    const candidates = []
 ...
 */
export function analyseRenderPhp(blockJsonPath, blockJson) {
    const result = {
        found: false,
        path: null,
        isDynamic: false,
 ...
 */
export function buildContentAssertions(renderAnalysis, exampleAttributes) {
 ...
 */
export function loadEditorConfig(blockJsonPath) {
 ...
 */
export function loadFrontendConfig(blockJsonPath, options = {}) {
    const blockJson = JSON.parse(fs.readFileSync(blockJsonPath, 'utf8'));
    const blockName = blockJson.name;
    const title = blockJson.title || blockName;
    const exampleAttributes = blockJson.example?.attributes || {};
