    ...
    test.describe(`${title} - Front End (Block Renderer API)`, () => {
        ...
        test('rendered HTML has correct structure', async ({ page, requestUtils }) => {
            ...
            if (htmlElements.includes('a')) {
                ...
                for (let i = 0; i < linkCount; i++) {
                    ...
                    expect(text || ariaLabel || ariaLabelledBy, `a[${i}] must have an accessible name`).toBeTruthy();
                }
            }
            ...
            if (htmlElements.includes('button')) {
                ...
                for (let i = 0; i < btnCount; i++) {
                    ...
                    expect(text || ariaLabel, `button[${i}] must have an accessible name`).toBeTruthy();
                }
            }
            ...
            if (htmlElements.includes('svg')) {
                ...
                for (let i = 0; i < svgCount; i++) {
                    ...
                    expect(hidden === 'true' || titleCount > 0, `svg[${i}] must have aria-hidden="true" or a <title> element`).toBe(true);
                }
            }
            ...
            if (htmlElements.includes('iframe')) {
                ...
                for (let i = 0; i < iframeCount; i++) {
                    ...
                    expect(iframeTitle === null || iframeTitle === void 0 ? void 0 : iframeTitle.trim(), `iframe[${i}] must have a title attribute`).toBeTruthy();
                }
            }
            ...
            if (htmlElements.includes('video') || htmlElements.includes('audio')) {
                ...
                for (let i = 0; i < mediaCount; i++) {
                    ...
                    expect(trackCount, `media element [${i}] must have at least one <track> for captions (WCAG 1.2)`).toBeGreaterThan(0);
                }
            }
            ...
            if (htmlElements.includes('dialog')) {
                ...
                if (dialogCount > 0) {
                    ...
                    expect(triggerCount, 'dialog block must have at least one trigger button').toBeGreaterThan(0);
                }
            }
        ...
        if (hasInteractiveDirectives) {
            ...
            test('interactive elements are keyboard accessible', async ({ page, requestUtils }) => {
                ...
                for (let i = 0; i < count; i++) {
                    ...
                    expect(tabIndex === null || tabIndex === '0' || tabIndex === '-1' || parseInt(tabIndex) > 0, `Interactive element [${i}] has unexpected tabindex: "${tabIndex}"`).toBe(true);
                }
            });
        }
        // -----------------------------------------------------------------------
        ...
        if (visualRegression) {
            test('block visual appearance matches baseline screenshot', async ({ page, requestUtils }) => {
                ...
                await expect(page.locator(`.${wrapperClass}`)).toHaveScreenshot(`${wrapperClass}.png`, {
