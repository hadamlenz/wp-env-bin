...
exports.registerFrontendTests = registerFrontendTests;
exports.registerFrontendTestsFromConfig = registerFrontendTestsFromConfig;
...
function logAxeViolations(blockTitle, violations) {
    if (!violations.length)
        return;
    console.log(`\nAxe violations for ${blockTitle}:`);
    violations.forEach((v) => {
        var _a;
        console.log(`  [${(_a = v.impact) === null || _a === void 0 ? void 0 : _a.toUpperCase()}] ${v.id}: ${v.description}`);
...
function isSkippableError(err) {
    return ((err === null || err === void 0 ? void 0 : err.status) === 400 ||
        (err === null || err === void 0 ? void 0 : err.status) === 404 ||
        (err === null || err === void 0 ? void 0 : err.code) === 'block_invalid_context' ||
        (err === null || err === void 0 ? void 0 : err.code) === 'block_invalid');
}
/**
 ...
 */
async function fetchAndRenderBlock(page, requestUtils, config) {
    ...
    (0, test_1.expect)(rendered.rendered.trim()).not.toBe('');
 ...
 */
function registerFrontendTests(test, config) {
    const { blockName, title, apiAttributes, htmlElements = [], ariaAttributes = [], contentAssertions = [], hasInteractiveDirectives = false, blockCss = null, screenshots = false, visualRegression = false, } = config;
    ...
    test.describe(`${title} - Front End (Block Renderer API)`, () => {
        ...
        test('block renderer API returns 200 with non-empty HTML', async ({ requestUtils }) => {
            ...
            (0, test_1.expect)(typeof rendered.rendered).toBe('string');
            (0, test_1.expect)(rendered.rendered.trim()).not.toBe('');
        });
        test('rendered HTML contains the block wrapper class', async ({ requestUtils }) => {
            ...
            (0, test_1.expect)(rendered.rendered).toContain(wrapperClass);
        });
        // -----------------------------------------------------------------------
        ...
        test('rendered HTML has correct structure', async ({ page, requestUtils }) => {
            ...
            await (0, test_1.expect)(block).toBeAttached();
            ...
            if (htmlElements.includes('img')) {
                ...
                for (let i = 0; i < imgCount; i++) {
                    ...
                    (0, test_1.expect)(alt, `img[${i}] must have an alt attribute`).not.toBeNull();
                }
            }
            ...
            if (htmlElements.includes('a')) {
                ...
                for (let i = 0; i < linkCount; i++) {
                    ...
                    (0, test_1.expect)(text || ariaLabel || ariaLabelledBy, `a[${i}] must have an accessible name`).toBeTruthy();
                }
            }
            ...
            if (htmlElements.includes('button')) {
                ...
                for (let i = 0; i < btnCount; i++) {
                    ...
                    (0, test_1.expect)(text || ariaLabel, `button[${i}] must have an accessible name`).toBeTruthy();
                }
            }
            ...
            if (htmlElements.includes('svg')) {
                ...
                for (let i = 0; i < svgCount; i++) {
                    ...
                    (0, test_1.expect)(hidden === 'true' || titleCount > 0, `svg[${i}] must have aria-hidden="true" or a <title> element`).toBe(true);
                }
            }
            ...
            if (htmlElements.includes('iframe')) {
                ...
                for (let i = 0; i < iframeCount; i++) {
                    ...
                    (0, test_1.expect)(iframeTitle === null || iframeTitle === void 0 ? void 0 : iframeTitle.trim(), `iframe[${i}] must have a title attribute`).toBeTruthy();
                }
            }
            ...
            if (htmlElements.includes('video') || htmlElements.includes('audio')) {
                ...
                for (let i = 0; i < mediaCount; i++) {
                    ...
                    (0, test_1.expect)(trackCount, `media element [${i}] must have at least one <track> for captions (WCAG 1.2)`).toBeGreaterThan(0);
                }
            }
            ...
            if (htmlElements.includes('dialog')) {
                ...
                if (dialogCount > 0) {
                    ...
                    (0, test_1.expect)(triggerCount, 'dialog block must have at least one trigger button').toBeGreaterThan(0);
                }
            }
            ...
            for (const attr of ariaAttributes) {
                ...
                for (let i = 0; i < attrCount; i++) {
                    ...
                    (0, test_1.expect)(val, `[${attr}] element [${i}] must have a value`).not.toBeNull();
                }
            }
            // Attribute value assertions from render.php cross-reference
            for (const { key, value } of contentAssertions) {
                (0, test_1.expect)(rendered, `${key} value should appear in rendered output`).toContain(value);
            }
            // wp-interactivity — data-wp-* directive presence
            if (hasInteractiveDirectives) {
                const interactiveEl = block.locator('[data-wp-interactive], [data-wp-bind], [data-wp-on], [data-wp-class], [data-wp-context]').first();
                await (0, test_1.expect)(interactiveEl).toBeVisible();
            }
        });
        test('rendered HTML is valid markup — wrapper is present in parsed DOM', async ({ page, requestUtils }) => {
            ...
            await (0, test_1.expect)(page.locator(`.${wrapperClass}`)).toBeAttached();
        });
        // -----------------------------------------------------------------------
        // Accessibility
        // -----------------------------------------------------------------------
        ...
        if (hasInteractiveDirectives) {
            ...
            test('interactive elements are keyboard accessible', async ({ page, requestUtils }) => {
                ...
                await page.waitForLoadState('domcontentloaded');
                await page.waitForTimeout(300);
                const interactiveEls = page.locator(`.${wrapperClass} button, .${wrapperClass} a, .${wrapperClass} input, .${wrapperClass} select, .${wrapperClass} [tabindex]`);
                const count = await interactiveEls.count();
                for (let i = 0; i < count; i++) {
                    const tabIndex = await interactiveEls.nth(i).getAttribute('tabindex');
                    (0, test_1.expect)(tabIndex === null || tabIndex === '0' || tabIndex === '-1' || parseInt(tabIndex) > 0, `Interactive element [${i}] has unexpected tabindex: "${tabIndex}"`).toBe(true);
                }
            });
        }
        // -----------------------------------------------------------------------
        ...
        if (visualRegression) {
            test('block visual appearance matches baseline screenshot', async ({ page, requestUtils }) => {
                ...
                await (0, test_1.expect)(page.locator(`.${wrapperClass}`)).toHaveScreenshot(`${wrapperClass}.png`, {
 ...
 */
function registerFrontendTestsFromConfig(test, configPath, options = {}) {
    var _a;
    const fs = require('fs');
    ...
    for (const dir of ((_a = cfg.frontend) !== null && _a !== void 0 ? _a : [])) {
        ...
        registerFrontendTests(test, (0, block_loader_1.loadFrontendConfig)(blockJsonPath, options));
    }
}
