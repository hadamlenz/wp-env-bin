...
const block_loader_js_1 = require("./block-loader.js");
// ---------------------------------------------------------------------------
// Private helpers
// ---------------------------------------------------------------------------
...
function registerFrontendTestsFromConfig(test, configPath, options = {}) {
    ...
    const root = path_1.default.resolve(path_1.default.dirname(configPath), '../..');
    for (const dir of (cfg.frontend ?? [])) {
        const blockJsonPath = path_1.default.resolve(root, dir, 'block.json');
        if (!fs_1.default.existsSync(blockJsonPath)) {
            console.warn(`[wp-env-bin] block.json not found at ${blockJsonPath} — skipping`);
            continue;
        }
        registerFrontendTests(test, (0, block_loader_js_1.loadFrontendConfig)(blockJsonPath, options));
    }
}
