...
import * as blockLoader from './block-loader.js';
 ...
 *
 * @param {{ files?: string[], output?: string, glob?: string|null, screenshots?: boolean, visualRegression?: boolean, help?: boolean }} options
 */
async function generate(options) {
  ...
  for (const file of uniqueFiles) {
    ...
    if (processFile(file, output, screenshots, visualRegression)) passed++;
    else failed++;
  }
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  generate(parseArgs(process.argv.slice(2))).catch(err => { console.error(err); process.exit(1); });
}
