...
const { readRawConfig } = require("../env/config");
...
const MENUS = {
	...
	advanced: {
		title: "Advanced",
		items: [
			{ label: "Composer Get",     hint: "fetch remote composer package",  id: "advanced:composer-get",  type: "action" },
			{ label: "Composer Make",    hint: "generate local composer.json",   id: "advanced:composer-make", type: "action" },
			{ label: "Make .htaccess",   hint: "generate reverse proxy config",  id: "advanced:htaccess",      type: "action" },
			{ label: "Scaffold Project", hint: "copy wp-env-bin scaffold files", id: "advanced:scaffold",      type: "action" },
	...
	return React.createElement(
		...
		items.map((item, i) => {
			...
			return React.createElement(
				...
				item.hint
					? React.createElement(Text, { color: "gray" }, "  " + item.hint)
					: null,
			);
		}),
	);
...
function ConfigView({ rows, cursor }) {
	...
	return React.createElement(
		...
		rows.length > CONFIG_VISIBLE
			? React.createElement(Text, { color: "gray" }, "  " + (start + CONFIG_VISIBLE < rows.length ? "▼ " + (rows.length - start - CONFIG_VISIBLE) + " more" : ""))
			: null,
	);
}
...
async function runAction(id) {
	...
	const { configCreatePrompt, configSwitchPrompt, configUpdatePrompt, configInstallPrompt } = require("./config");
	...
	const { runWpEnv, runWpEnvE2e } = require("../../commands/env");
	...
	switch (id) {
		case "env:sync-start":  await envSyncPrompt(); runWpEnv(["start"]); break;
		case "env:sync":        await envSyncPrompt(); break;
		case "env:start":       runWpEnv(["start"]); break;
		case "env:stop":        runWpEnv(["stop"]); break;
		case "env:restart":     runWpEnv(["stop"]); runWpEnv(["start"]); break;
		case "db:get":          await dbGetPrompt(); break;
		case "db:process":      await dbProcessPrompt(); break;
		case "db:fresh":        await dbGetPrompt(); await dbProcessPrompt(); break;
		case "config:create":   await configCreatePrompt(); break;
		case "config:switch":   await configSwitchPrompt(); break;
		case "config:update":   await configUpdatePrompt(); break;
		case "config:install":  await configInstallPrompt(); break;
		case "e2e:scaffold":    await e2eScaffoldPrompt(); break;
		case "e2e:generate":    generateE2eTests([]); break;
		case "e2e:test":        runE2eTests([]); break;
		case "advanced:composer-get":  await composerGetPrompt(); break;
		case "advanced:composer-make": await composerMakePrompt(); break;
		case "advanced:htaccess":      await htaccessMakePrompt(); break;
		case "advanced:scaffold":      scaffoldCommand(path.join(process.cwd(), "wp-env-bin")); break;
		case "advanced:visual":        await visualComparePrompt([]); break;
		case "advanced:clean":         cleanAll(); break;
		case "e2e-env:start":          runWpEnvE2e(["start"]); break;
		case "e2e-env:stop":           runWpEnvE2e(["stop"]); break;
		case "e2e-env:restart":        runWpEnvE2e(["stop"]); runWpEnvE2e(["start"]); break;
		case "e2e-env:update":         runWpEnvE2e(["start", "--update"]); break;
	}
}
