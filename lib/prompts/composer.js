...
import { composerMake } from "../../commands/composer.js";
import { getProfileList, getActiveProfile } from "../../commands/config.js";
import { matchActivePlugins, buildComposerJson, fetchRemoteData, makeComposerName } from "../remote-composer.js";

async function composerGetPrompt(runtimePath, runtimeUrl) {
	...
	const siteConfigsDir = path.join(process.cwd(), "wp-env-bin", "site-configs");

	if (activeProfile) {
		const existingComposer = path.join(siteConfigsDir, activeProfile + ".composer.json");
		if (existsSync(existingComposer)) {
			console.log(chalk.yellow(`> Note: site-configs/${activeProfile}.composer.json already exists.`));
		}
	}

	const chosenComp = await select({
		message: runtimeUrl
			? "Save fetched composer.json to which profile?"
			: "Build composer.json for which profile?",
		choices: profiles.map(p => ({
			name: activeProfile && p === activeProfile ? `${p} (config loaded)` : p,
			value: p,
		})),
		default: activeProfile && profiles.includes(activeProfile) ? activeProfile : undefined,
	});
	...
	if (runtimeUrl) {
		...
		if (saveUrl) {
			...
			const copyActive = await confirm({ message: "Also copy to active wp-env-bin/composer.json?", default: false });
			if (copyActive) {
	...
	const saveComp = await confirm({ message: "Save as site-configs/" + chosenComp + ".composer.json?", default: true });
	if (saveComp) {
...
async function composerMakePrompt() {
	const profiles = getProfileList();
	if (!profiles || profiles.length === 0) {
		console.log(chalk.red("No profiles found in wp-env-bin/site-configs/."));
		console.log("Run `wp-env-bin config create` to create one.");
	...
	const chosenMake = await select({
		message: "Create blank composer.json for which profile?",
		choices: profiles.map(p => ({ name: p, value: p })),
	});
	composerMake(chosenMake);

	const copyActiveMake = await confirm({ message: "Also copy to active wp-env-bin/composer.json?", default: false });
	if (copyActiveMake) {
...
export { composerGetPrompt, composerMakePrompt };
