...
const { matchActivePlugins, buildComposerJson, fetchRemoteData } = require("../remote-composer");
...
async function handleComposerGet(runtimePath, runtimeUrl) {
	...
	if (saveComp) {
		const composerJson = buildComposerJson(filteredMatched, serverComposer.repositories || [], filteredThemePkg ? { [filteredThemePkg]: "*" } : null);
